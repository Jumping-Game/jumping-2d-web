...
```json
{
  ...
  "seq": 201,
  "ts": 1737767,
  "payload": { "t0": 1737767123456, "t1": 1737767123470 }
}
```
...
```json
{
  ...
  "seq": 9,
  "ts": 1737765,
  "payload": { "code": "BAD_VERSION", "message": "Server pv=1, client pv=0" }
}
```
...
```json
{
  ...
  "seq": 300,
  "ts": 1737770,
  "payload": { "id": "p_efgh", "state": "disconnected" }
}
```
...
```json
{
  ...
  "payload": {
    ...
    "full": false,
    "players": [
      { "id": "p_abcd", "x": 321.1, "y": 1120.5, "vx": 9.0, "vy": 1180.0 }
    ]
  }
}
```
...
- Same `seed` + scripted inputs for 10 k ticks on ARM64 Android vs x86_64 Linux → **bit-equal** world hash every 1 k ticks.
...
interface C2S_InputBatch {
  startTick: number;
  frames: Array<{ d: number; axisX: number; jump?: boolean; shoot?: boolean }>;
}

interface C2S_Ping {
  t0: number;
}
interface C2S_Reconnect {
  playerId: string;
  resumeToken: string;
  lastAckTick: number;
}
...
interface S2C_Finish {
  reason: 'room_closed' | 'timeout' | 'error';
}
interface S2C_PlayerPresence {
  id: string;
